...
class Repos:
    ...
    def __init__(self, root:str, absolute=True, recurse=True) -> None:
        self.root = Path(root)
        self.recurse = recurse
        self.absolute = absolute
        self.repos = self._scan()
    ...
    def _scan(self):
        ...
        '''
        path = self.root.resolve() if self.absolute else self.root
        if self.recurse:
            for repo_git_folder in path.glob('**/.git/'):
                yield git.get_repo(repo_git_folder.parent)
        else:
            try:
    ...
    def __str__(self):
        return f'Repos({self.root=}, {self.recurse=}, {self.absolute=})'
